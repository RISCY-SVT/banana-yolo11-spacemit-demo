...
Final production handoff package
...
- Production handoff is complete: open P0/P1 are none.
- Primary visual path: dynamic640 INT8 on `rt201`.
- Fast-live path: vendor320 INT8 on `rt123`.
- Metrics are reported by class; they are not one shared FPS leaderboard.
...
The release includes visual/semantic sanity and output consistency evidence. It does not include a full COCO mAP validation.
...
- Fresh clone build/deploy passed on Day 4.
- Loader proof passed: production binaries use repo-local runtime/OpenCV, not system `/lib/libonnxruntime.so.1`.
- GitHub and private GitLab mirror both published.
...
- Production tag: `production-2026-07-02`
- Open P0/P1: none
- Recommended handoff: use the tagged release and release docs as the operator package.
