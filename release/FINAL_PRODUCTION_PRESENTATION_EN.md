...
Banana YOLO11 SpacemiT Demo — Final COCO mAP and FPS Evidence
...
- Runtime/model policy unchanged; tag not moved.
...
- Primary image and normal camera: dynamic640 INT8 on `rt201`.
- Fast-live and trusted vendor320 visual: vendor320 INT8 on `rt123`.
- Vendor320 raw `rt201`: perf-only.
- FP16: experimental. `rt202` and YOLO26: not production.
...
- mAP measured only for production visual candidates.
- Metric classes are separate workloads.
- Camera FPS depends on live camera conditions.
