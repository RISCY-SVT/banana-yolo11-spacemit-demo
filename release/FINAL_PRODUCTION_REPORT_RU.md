# Финальный production-отчёт: Banana YOLO11 SpacemiT Demo
...
Демо Banana YOLO11 SpacemiT готово к production handoff для Banana-Pi BPI-F3 / SpacemiT K1X в рамках замороженной release-ссылки `production-2026-07-02`. Принятый production-исходник: `9c0933be58ee122389d1a43f45f81e80655d6904`. Этот пакет отчёта является только документацией: он не перемещает production tag и не меняет runtime/model policy.

Релиз поддерживает три операторских сценария: основной image/normal-camera visual detection с generated dynamic640 INT8 на `rt201`, responsive fast-live camera detection с vendor320 INT8 на `rt123`, и trusted vendor320 visual checks на `rt123`. Raw vendor320 `rt201` остаётся только low-latency benchmark branch. FP16 и YOLO26 остаются вне production.

Open P0: none. Open P1: none. Drive mirror verification зафиксирован как `verified-quick-by-operator`.

## Аппаратная и программная среда

Целевая плата: Banana-Pi BPI-F3 со SpacemiT K1X / X60. Канонический host/container workspace root: `/data`; board target: `svt@banana`. Cross toolchain: `/data/SpacemiT/spacemit-toolchain-linux-glibc-x86_64-v1.1.2`; base sysroot: `${TOOLCHAIN_ROOT}/sysroot`; overlay sysroot: `/data/sysroots/k1x-gtk3-overlay`; ISA/ABI baseline: `-march=rv64gcv_zvfh -mabi=lp64d`.

Правила релиза остаются прежними: перед build/deploy выполнять `source /data/build_scripts/01-env.sh`, держать base sysroot read-only, использовать overlay/staged dependencies для OpenCV и board libraries, и не копировать полный board `/usr` в sysroot.

## Production scope и runtime policy
...
Эта policy намеренно консервативна. Stable `rt202` был staged и протестирован, но aborts на текущих production paths, поэтому не принят. YOLO26n остаётся только P2 research candidate.

## Архитектура приложения

Демо представляет собой небольшой C++/scripted deployment вокруг ONNX Runtime со SpaceMIT Execution Provider. Shell helpers выбирают runtime tag, model, camera/display/headless поведение и benchmark mode. C++ app выполняет preprocessing, ORT session execution, YOLO decode/postprocess, annotation и output/log generation.

![Architecture diagram](assets/architecture_diagram.svg)

Production camera path использует стабильный `/dev/v4l/by-id` selection, когда он доступен. Fast-live является отдельной веткой с 320 letterbox model/runtime pair, оптимизированной под отзывчивость.

## Model and Runtime Artifacts

Runtime provenance закреплён в `third_party_manifest/runtime.lock` и суммирован в `release/RUNTIME_MANIFEST.md`:
...
Model provenance закреплён или описан в `third_party_manifest/models.lock`, `models/README.md` и `release/MODEL_MANIFEST.md`:
...
Релиз не сводит разные измерения в один leaderboard. Ниже приведена metric taxonomy из `docs/FPS_SUMMARY.md`.
...
Для финальной таблицы использованы свежие Day 1/Day 2 regression evidence и Day 4 final acceptance evidence. Ограниченные Day 4 smoke numbers трактуются как smoke evidence, а не как замена full regression tables.

## Performance Summary

Ниже приведены representative production and experimental rows. Полные детали находятся в appendix и в `docs/FPS_SUMMARY.md` / `release/FPS_SUMMARY.md`.
...
Релиз содержит visual/semantic sanity и output consistency evidence, но не full COCO mAP validation. Никаких claims по full COCO mAP не делается.

Day 4 final acceptance прошёл image, normal camera, fast-live, headless, FP16 experimental smoke, loader proof, Doxygen и hygiene checks. Основной still-image path обнаружил 14 objects в final Day 4 dynamic640 smoke; vendor320 trusted visual path обнаружил 8 objects в final Day 4 smoke.
...
Normal camera demo использует generated dynamic640 INT8 на `rt201`. Day 4 final acceptance прошёл fresh-clone и current-repo camera smoke. Ограниченный финальный smoke зафиксировал stable by-id camera selection, MJPG capture, effective 2.069 FPS over 10 frames и instantaneous frame timing 238.716 ms / 4.189 FPS.

![Normal camera Day 4 still](assets/camera_default_day4.jpg)

Headless mode явно поддерживается через `HEADLESS_FLAG=1`; Day 4 headless smoke показал progress logs и отсутствие hang.

## Fast-live Mode

Fast-live является веткой responsiveness. Она использует vendor320 INT8 на `rt123` с 320 letterbox. Day 4 final acceptance прошёл fast-live smoke с effective 9.382 FPS over 20 frames и frame-20 instantaneous timing 67.121 ms / 14.898 FPS.

![Fast-live Day 4 still](assets/camera_fast_day4.jpg)

## Loader / Deployment / Reproducibility Evidence

Day 4 fresh-clone proof прошёл `fetch_vendor_runtime`, `fetch_models`, cross-build, deploy, image demo, normal camera, fast-live и headless smoke. Loader integrity прошёл для `app/bin/banana_yolo11_demo`, `bin/banana_yolo11_demo_rt201` и `bin/banana_yolo11_demo_rt123`; production binaries резолвили repo-local deployed runtime/OpenCV libraries и не резолвили system `/lib/libonnxruntime.so.1`.

Репозиторий опубликован в GitHub и private GitLab mirror. Production tag `production-2026-07-02` является annotated tag, указывающим на `9c0933be58ee122389d1a43f45f81e80655d6904`. Host Drive verification предоставлен оператором как `VERIFY=quick` с нулём differences в синхронизированных roots.

## Known Limitations

Ограничения приняты и задокументированы: stable `rt202` staged, но не adopted; FP16 остаётся experimental only; FP16 320 unsupported; vendor320 raw `rt201` perf-only; vendor320 `rt201` visual workaround SHA256-guarded и non-default; full COCO mAP validation не выполнялся; camera performance зависит от camera mode, display/headless state и live-loop conditions; latest-frame/drop-old-frames pipeline work явно out of scope для релиза.

## Rejected / Experimental Paths

Rejected или non-production paths остаются: stable `rt202` dynamic640/FP16/vendor320 aborts; YOLO26n float и dynamic INT8 отклонены для production из-за semantic failure и остаются P2 only; true FP16 I/O models являются dtype evidence, но не supported board chain; FP16 keep_io 640 на `rt201`/`rt202b1` usable, но experimental; official public 640 INT8 artifact search и camera pipeline R&D остаются P2.
...
Следующая таблица скопирована из authoritative `docs/FPS_SUMMARY.md`, где есть evidence source по строкам.
...
Дополнительные release files: `release/PRODUCTION_READINESS_REPORT.md`, `release/PRODUCTION_CHECKLIST.md`, `release/DEMO_COMMANDS.md`, `release/RUNTIME_MANIFEST.md`, `release/MODEL_MANIFEST.md`, `release/BUILD_MANIFEST.md`, `release/KNOWN_LIMITATIONS.md`, `release/HANDOFF_NOTES.md` и `release/FPS_SUMMARY.md`.
